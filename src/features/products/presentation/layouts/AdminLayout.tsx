...
import {
  ...
  MessageCircle,
  Users,
  Layers, // <-- Đã thêm icon Layers ở đây
} from "lucide-react";
...
export const AdminLayout: React.FC = () => {
  ...
  const [isFeedbackOpen, setIsFeedbackOpen] = useState(
    ...
    location.pathname.startsWith("/admin/chat-feedbacks"),
  );
  ...
  const adminMenuItems: MenuItem[] = [
    ...
    // --- ĐÃ THÊM MỤC WASH BAYS VÀO ĐÂY ---
    {
      path: "/admin/wash-bays",
      label: t('adminSidebar.washBays', { defaultValue: 'Wash Bays' }),
      icon: (isActive) => (
        <div
          className={`p-2 rounded-xl transition-colors ${isActive ? "bg-white/20" : "bg-slate-100 dark:bg-white/5 group-hover:bg-white dark:group-hover:bg-white/10"}`}
        >
            ...
            {adminMenuItems.map((item, idx) => {
              if (item.children) {
                ...
                return (
                  <div key={idx} className="flex flex-col w-full">
                    ...
                    <div
                      className={`overflow-hidden transition-all duration-300 pl-11 space-y-1 mt-1.5 ${isFeedbackOpen ? "max-h-32 opacity-100" : "max-h-0 opacity-0"}`}
                    >
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path!}
                  className={`group flex items-center gap-3.5 px-3 py-3 rounded-2xl font-bold text-sm transition-all duration-300 ${isActive
