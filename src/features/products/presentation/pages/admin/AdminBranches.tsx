...
export function AdminBranches() {
  ...
  const [selectedBranchForPromo, setSelectedBranchForPromo] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  const handleCreate = async () => {
    if (editForm && editForm.branchName && editForm.address) {
      try {
        ...
        const newBranchId =
          response?.id || response?.data?.id || "NEW_BRANCH_ID";
  ...
  const handleSkipPromotion = () => {
    if (
      window.confirm(
        "Bạn có chắc không có chương trình khuyến mãi ở chi nhánh mới?",
      )
    ) {
  ...
  const handleCreateNewPromotion = async () => {
    ...
    if (
      !newPromoForm.code ||
      !newPromoForm.name ||
      !newPromoForm.startDate ||
      !newPromoForm.endDate
    ) {
      alert(
        "Vui lòng điền các trường bắt buộc (Mã, Tên, Ngày bắt đầu, Ngày kết thúc)!",
      );
      return;
    }
    ...
    try {
      ...
      alert(
        `Đã tạo và kích hoạt khuyến mãi mới cho chi nhánh: ${selectedBranchForPromo.name}`,
      );
  ...
  const handleLinkPromotion = async () => {
    ...
    try {
      ...
      const updatedBranchIds = Array.from(
        new Set([...currentBranchIds, selectedBranchForPromo.id]),
      );
      ...
      alert(
        `Đã liên kết Khuyến mãi thành công với Chi nhánh: ${selectedBranchForPromo.name}`,
      );
      setSelectedBranchForPromo(null);
      setSelectedPromoId("");
    } catch (error) {
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-2xl font-bold text-gray-900">
              {t("adminBranches.title", { defaultValue: "Branches" })}
            </h3>
            <p className="text-gray-500">
              {t("adminBranches.subtitle", {
                defaultValue: "Manage all car wash branches locations",
              })}
            </p>
          </div>
          <button
          ...
          >
            ...
            {t("adminBranches.addBranch", { defaultValue: "Add Branch" })}
          </button>
        </div>
              ...
              >
                ...
                <div className="space-y-3 mb-6">
                  <div className="flex items-start gap-2">
                    ...
                    <span className="text-sm text-gray-600 line-clamp-2">
                      {i18n.language.startsWith("en")
                        ? removeVietnameseTones(
                            branch.address.replace(
                              "Vietnam",
                              t("adminBranches.vietnam", {
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-600">
                      {t("adminBranches.hours", { defaultValue: "Hours:" })}{" "}
                ...
                >
                  {branch.status === "Active" ? (
                    <span className="flex items-center justify-center gap-2">
                      ...
                      {t("adminBranches.active", { defaultValue: "Active" })}
                    </span>
                  ) : (
                    <span className="flex items-center justify-center gap-2">
                      <XCircle className="w-4 h-4" />
                      {t("adminBranches.inactive", {
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-5xl shadow-2xl max-h-[90vh] overflow-y-auto relative">
            <div className="flex items-center justify-between mb-6">
              <h4 className="text-xl font-bold text-gray-900">
                {isAdding
                  ? t("adminBranches.addNewBranch", {
                      defaultValue: "Add New Branch",
                    })
                  : t("adminBranches.editBranch", {
                      defaultValue: "Edit Branch",
                    })}
              </h4>
              <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      ...
                      <span className="text-[10px] font-bold tracking-wider uppercase">
                        {t("adminBranches.searchMap", {
                          defaultValue: "Search Map",
                        })}
                      </span>
                    </button>
                  </div>
                  <p className="text-[11px] text-gray-500 italic mt-1.5 leading-relaxed">
                    {t("adminBranches.addressHelper", {
                      defaultValue:
                        '* Enter detail address and click "Search Map" to sync coordinates automatically.',
                    })}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      {t("adminBranches.hotline", { defaultValue: "Hotline" })}
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      {t("adminBranches.operatingHoursLabel", {
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder={t("adminBranches.selectOnMap", {
                        defaultValue: "Select on map",
                      })}
                    />
                  </div>
                  <div>
              ...
              <div className="flex flex-col h-[400px] lg:h-auto border border-gray-300 rounded-lg overflow-hidden">
                <div className="p-3 bg-gray-50 border-b border-gray-300 text-sm text-gray-600 flex items-center gap-2">
                  ...
                  {t("adminBranches.mapHint", {
                    defaultValue:
                      'Click on the map to pin the location or use the "Search Map" button next to the address',
                  })}
                </div>
            ...
            <div className="flex justify-end gap-3 pt-6 mt-6 border-t border-gray-100">
              ...
              >
                {t("adminBranches.cancel", { defaultValue: "Cancel" })}
              </button>
              <button
              ...
              >
                ...
                {isAdding
                  ? t("adminBranches.createBranch", {
                      defaultValue: "Create Branch",
                    })
                  : t("adminBranches.saveChanges", {
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl animate-scale-up max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
                  ...
                  {washBaysList.map((wbName, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <input
                        ...
                        value={wbName}
                        onChange={(e) =>
                          handleUpdateWashBayName(index, e.target.value)
                        }
              ...
              <div className="flex justify-between items-center pt-4 mt-6 border-t border-gray-100">
                ...
                >
                  {isCreatingStaff || isCreatingWashBay
                    ? "Đang tạo..."
                    : "Xác nhận & Tiếp tục"}
                </button>
              </div>
      ...
      {selectedBranchForPromo && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div
            className={`bg-white rounded-xl p-6 w-full shadow-2xl transition-all ${isCreatingNewPromo ? "max-w-2xl" : "max-w-md"}`}
          >
            <div className="flex items-center justify-between mb-4 border-b border-gray-100 pb-3">
              <h4 className="text-lg font-bold text-gray-900">
                Bước 3:{" "}
              ...
              {!isCreatingNewPromo && (
                <button
                  onClick={() => setSelectedBranchForPromo(null)}
                  className="text-gray-400 hover:text-gray-600"
                >
                  <X className="w-5 h-5" />
                </button>
              ...
              <div className="grid grid-cols-2 gap-4 max-h-[60vh] overflow-y-auto pr-2">
                <div className="col-span-2 sm:col-span-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewPromoForm({ ...newPromoForm, code: e.target.value })
                    }
                    className="w-full px-3 py-2 border rounded-lg text-sm"
                    placeholder="VD: KHAI_TRUONG_Q9"
                  />
                </div>
                <div className="col-span-2 sm:col-span-1">
                  <label className="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewPromoForm({ ...newPromoForm, name: e.target.value })
                    }
                    className="w-full px-3 py-2 border rounded-lg text-sm"
                  ...
                  {promotions
                    ...
                    .map((promo) => (
                      <option key={promo.id} value={promo.id}>
                        {(promo as any).name || promo.description} (Giảm:{" "}
                        {promo.discountValue})
                      </option>
                    ))}
                </select>
