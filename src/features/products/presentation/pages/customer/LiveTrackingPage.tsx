import React, { useEffect } from "react"; // 💡 Bổ sung import useEffect
...
import {
  ...
  ArrowLeft,
  Activity,
  type LucideIcon, // 💡 Bổ sung import LucideIcon
  // 💡 Bổ sung import LucideIcon
} from "lucide-react";
import { useBooking } from "@/features/products/application/useBooking"; // (Giữ nguyên đường dẫn tuyệt đối từ lỗi trước)
...
interface StatusStep {
  id: WashStatus[];
  label: string;
  icon: LucideIcon; // 💡 Đổi từ React.ElementType sang LucideIcon
  description: string;
}
...
const washSteps: StatusStep[] = [
  {
    ...
    label: "Checked In",
    icon: CheckCircle2,
    description: "Vehicle checked in at facility",
  },
  {
    id: ["Queued"],
    label: "In Queue",
    icon: Clock,
    description: "Waiting for available bay",
  },
  {
    id: ["InProgress"],
    label: "Washing & Drying",
    icon: Droplets,
    description: "Active wash in progress",
  },
  {
    id: ["Completed", "CheckedOut"],
    label: "Completed",
    icon: Sparkles,
...
export const LiveTrackingPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!bookingId) {
      console.warn(
        "LiveTrackingPage: Missing bookingId, redirecting to history.",
      );
      ...
      <div className="max-w-md mx-auto mt-20 text-center space-y-6 p-8 bg-white rounded-3xl border border-slate-100 shadow-xl">
        ...
        <div>
          ...
          <p className="text-slate-500 font-medium mt-2">
            We couldn't find the tracking information for this booking.
          </p>
        </div>
        <button
  ...
  const getProgressPercentage = () => {
    ...
    if (index === -1) return 0;
    if (currentStatus === "Completed" || currentStatus === "CheckedOut")
      return 100;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-gradient-to-br from-[#1e6ffd] to-[#1d4ed8] rounded-[48px] p-10 text-white relative overflow-hidden shadow-2xl shadow-blue-500/20">
            ...
            <div className="relative z-10 space-y-10">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  <span className="text-blue-100 text-[10px] font-black uppercase tracking-[0.2em] opacity-80">
                    Order Tracking
                  </span>
                  <h1 className="text-4xl font-black tracking-tight mt-1">
                    #BW-{booking.bookingCode}
                  </h1>
                </div>
              ...
              <div className="space-y-4">
                ...
                <p className="text-blue-50 font-bold opacity-90 flex items-center gap-2">
                  ...
                  Current Stage:{" "}
                  <span className="text-white font-black">{currentStatus}</span>
                </p>
              </div>
            </div>
          </div>
                ...
                return (
                  <div key={index} className="flex gap-8 relative">
                    ...
                    {index < washSteps.length - 1 && (
                      <div
                        className={`absolute left-7 top-14 w-1 h-12 rounded-full ${completed ? "bg-emerald-500" : "bg-slate-100"}`}
                      ></div>
                    )}

                    <div
                      className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 transition-all duration-500 shadow-xl ${
                        completed
                          ? "bg-emerald-500 text-white shadow-emerald-100"
                          : active
                            ? "bg-blue-600 text-white shadow-blue-100 scale-110 rotate-3 animate-pulse"
                    ...
                    <div className="flex-1 pt-1">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {active && (
                          <span className="px-3 py-1 bg-blue-50 text-blue-600 text-[10px] font-black rounded-lg uppercase tracking-widest animate-pulse">
                            Processing
                          </span>
                        )}
                      </div>
                      <p
                        className={`text-sm font-bold ${active || completed ? "text-slate-500" : "text-slate-300"}`}
                      >
                        {step.description}
                      </p>
                    </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 rounded-[36px] p-8 text-white space-y-4 relative overflow-hidden group">
            ...
            <h3 className="text-lg font-black tracking-tight relative z-10">
              Need Help?
            </h3>
            <p className="text-xs text-slate-400 font-bold leading-relaxed relative z-10">
              If you have any questions about your wash progress, feel free to
          ...
          {(currentStatus === "Completed" ||
            ...
            <div className="bg-emerald-50 border-2 border-emerald-500 rounded-[36px] p-8 space-y-4 animate-in zoom-in duration-500">
              ...
              <p className="text-sm font-bold text-emerald-700 leading-relaxed">
                Your vehicle is ready for pickup. Please show your booking code{" "}
