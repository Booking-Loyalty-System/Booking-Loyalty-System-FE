        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <form onSubmit={onSubmit} className="p-6 space-y-4 overflow-y-auto flex-1">
                    {/* License Plate */}
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">License Plate *</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Vehicle Name *</label>
                    ...
                    <div>
                        ...
                        <select name="type" value={formData.type} onChange={handleInputChange} className="w-full px-4 py-2.5 rounded-xl border border-slate-200 font-bold text-slate-900 bg-white">
                            ...
                            <option value="Medium">Medium (SUV / CUV)</option>
                            <option value="Large">Large (Pickup)</option>
                        </select>
                    </div>
                    ...
                    <div className="flex items-center gap-2 pt-2">
                        ...
                        <label htmlFor="isPrimary" className="text-sm font-bold text-slate-600">Set as primary vehicle</label>
                    </div>

                    <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 mt-6">
                        <button type="button" onClick={onClose} className="px-5 py-2.5 rounded-xl text-sm font-bold text-slate-500 hover:bg-slate-100">Cancel</button>
                        <button type="submit" disabled={isCreating} className="inline-flex items-center gap-1.5 bg-blue-600 text-white font-bold text-sm px-6 py-2.5 rounded-xl">
                            ...
                            <span>{isCreating ? 'Saving...' : 'Save Vehicle'}</span>
                        </button>
                    </div>
                </form>
            </div>
