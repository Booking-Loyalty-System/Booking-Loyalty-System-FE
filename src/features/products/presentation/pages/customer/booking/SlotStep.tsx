...
export const SlotStep: React.FC<SlotStepProps> = ({
  branchId,
  onSelect,
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetAvailableSlots(branchId, formattedDate);

  // Helper: Tạo danh sách các ngày trong khung cho phép
  const dates = useMemo(() => {
    return Array.from({ length: bookingWindow }, (_, i) =>
      addDays(new Date(), i),
    );
  }, [bookingWindow]);
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-gray-500">
          ...
          <span className="text-blue-600 font-bold">
            {loyaltyInfo?.currentTier || "Member"}
          </span>
          , you have a{" "}
          <span className="font-bold text-gray-900">{bookingWindow}-day</span>{" "}
          booking window.
        </p>
      </div>
        ...
        <div className="flex gap-3 overflow-x-auto pb-4 no-scrollbar">
          {dates.map((date) => {
            ...
            return (
              ...
              >
                <span
                  className={`block text-[10px] font-bold uppercase tracking-widest ${isSelected ? "text-blue-600" : "text-gray-400"}`}
                >
                  {format(date, "EEE")}
                </span>
                <span
                  className={`block text-xl font-black mt-1 ${isSelected ? "text-blue-600" : "text-gray-900"}`}
                >
                  {format(date, "dd")}
                </span>
                <span
                  className={`block text-[10px] font-medium mt-1 ${isSelected ? "text-blue-400" : "text-gray-400"}`}
                >
                  {format(date, "MMM")}
                </span>
              </div>
            ...
            {slots?.map((slot) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span
                      className={`font-bold ${isSelected ? "text-blue-600" : "text-gray-900"}`}
                    >
                      {slot.time}
                    </span>
                    {isFull && (
                  ...
                  <div className="mt-1 flex items-center justify-between">
                    ...
                    <div className="flex gap-0.5">
                      {Array.from({ length: 3 }).map((_, i) => (
                        <div
                          key={i}
                          className={`w-1 h-1 rounded-full ${i < slot.maxCapacity - slot.currentBookings ? "bg-emerald-400" : "bg-gray-200"}`}
                        />
                      ))}
                    </div>
                  </div>
