...
export const Dashboard: React.FC = () => {
  ...
  const activeBooking =
    bookings.find((b) =>
      ["InProgress", "Washing", "Drying"].includes(b.status),
  ...
  const stats = [
    {
      ...
      value: bookings
        .filter(
          (b) => new Date(b.bookingDate).getMonth() === new Date().getMonth(),
        )
        .length.toString(),
      sub: "Total Bookings",
    ...
    },
    {
      id: 2,
      label: "Loyalty",
      value: loyaltyInfo?.totalPoints.toString() || "850",
      sub: "Total Points",
    ...
    },
    {
      id: 3,
      label: "Status",
      value: loyaltyInfo?.currentTier || "Gold",
      sub: "Membership Tier",
    ...
    },
    {
      id: 4,
      label: "Savings",
      value: `$${(bookings.length * 5 + 127).toFixed(0)}`,
      sub: "Total Saved",
    ...
    <div className="space-y-8 max-w-7xl mx-auto pb-12 animate-in fade-in duration-700">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-blue-100 font-medium">
              Get 20% off for all Premium packages this Saturday & Sunday. Use
              code: WEEKEND20
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-sm text-[#64748b] font-bold">
            Your car wash command center is ready.
          </p>
        </div>
        ...
        <div className="bg-white border-2 border-blue-50 rounded-[40px] p-10 shadow-2xl shadow-blue-500/5 relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-center gap-10">
            <div className="shrink-0 flex flex-col items-center">
              ...
              <span className="mt-6 px-4 py-1.5 bg-blue-50 text-blue-700 text-[10px] font-black uppercase tracking-[0.2em] rounded-full border border-blue-100">
                Live Operation
              </span>
            </div>
            ...
            <div className="flex-1 space-y-8">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <span className="px-2 py-0.5 bg-emerald-100 text-emerald-700 text-[10px] font-black rounded uppercase">
                      In Progress
                    </span>
                    <span className="text-slate-300">•</span>
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                      Order #BW-{activeBooking.bookingCode}
                    </span>
                  </div>
                  ...
                  <p className="text-slate-500 font-bold mt-1 text-lg">
                    ...
                    <span className="text-blue-600">
                      {activeBooking.vehicleName || "Vehicle"}
                    </span>{" "}
                    is currently at the Drying station.
                  </p>
                </div>
              ...
              <div className="grid grid-cols-4 gap-4 relative pt-2">
                ...
                <div className="flex flex-col items-center gap-4">
                  <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-blue-100 transition-transform hover:scale-110">
                ...
                <div className="flex flex-col items-center gap-4">
                  <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-blue-200 animate-bounce">
                ...
                <div className="flex flex-col items-center gap-4">
                  <div className="w-12 h-12 bg-slate-100 rounded-2xl flex items-center justify-center text-slate-300 transition-transform hover:scale-110">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat) => (
          <div
            key={stat.id}
            className="bg-white border border-slate-100 rounded-[28px] p-7 flex flex-col justify-between min-h-[160px] relative group hover:shadow-2xl hover:shadow-slate-200/50 hover:-translate-y-2 transition-all duration-500 overflow-hidden"
          >
            ...
            <div className="flex items-center justify-between relative z-10">
              <span className="text-[10px] text-[#94a3b8] font-black tracking-[0.15em] uppercase">
                {stat.label}
              </span>
              <div
                className={`w-12 h-12 ${stat.bg} rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 shadow-sm`}
              >
                {stat.icon}
              </div>
            </div>
            <div className="mt-6 relative z-10">
              <span className="block text-4xl font-black text-[#0f172a] tracking-tight">
                {stat.value}
              </span>
              <span className="text-[11px] text-[#64748b] font-bold uppercase tracking-widest mt-1 opacity-70">
                {stat.sub}
              </span>
            </div>
          </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
          <div className="flex gap-6 items-center">
            ...
            <div>
              ...
              <p className="text-sm text-[#166534] font-bold opacity-80 mt-1">
                Complete 7 washes to earn a FREE session!
              </p>
            </div>
          </div>
          <div className="text-right bg-white/50 backdrop-blur-sm px-6 py-3 rounded-2xl border border-white">
            ...
            <span className="text-[10px] text-[#166534] font-black uppercase tracking-widest mt-1 block">
              Completed
            </span>
          </div>
        </div>
        ...
        <div className="space-y-4 relative z-10">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 pt-2">
            <p className="text-base font-bold text-[#166534]">
              Just{" "}
              <span className="text-[#16a34a] font-black px-2 py-1 bg-white rounded-lg mx-1 shadow-sm">
                2 more
              </span>{" "}
              washes to unlock your reward!
            </p>
            ...
            <div className="flex items-center gap-2">
              {[1, 2, 3, 4, 5].map((idx) => (
                <div
                  key={idx}
                  className="w-9 h-9 bg-white rounded-xl flex items-center justify-center shadow-sm border border-emerald-100 text-emerald-600"
                >
              ...
              {[6, 7].map((idx) => (
                <div
                  key={idx}
                  className="w-9 h-9 rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center text-xs font-black text-slate-400"
                >
                  {idx}
                </div>
              ))}
        ...
        <div className="lg:col-span-2 bg-gradient-to-br from-[#1e6ffd] via-[#2563eb] to-[#1d4ed8] rounded-[48px] p-12 text-white relative overflow-hidden shadow-2xl shadow-blue-500/20 min-h-[420px] flex flex-col justify-between group">
          ...
          <div className="space-y-10 relative z-10">
            <div className="flex justify-between items-start">
              <div className="flex items-center gap-8">
                ...
                <div>
                  <span className="block text-xs text-blue-100 font-black tracking-[0.3em] uppercase mb-2 opacity-80">
                    Membership Tier
                  </span>
                  <h2 className="text-5xl font-black tracking-tight">
                    {loyaltyInfo?.currentTier || "Gold"} Member
                  </h2>
                </div>
              </div>
            ...
            <div className="space-y-6 max-w-lg">
              ...
              <div className="w-full bg-white/10 h-5 rounded-3xl overflow-hidden backdrop-blur-md p-1.5 border border-white/5">
                <div
                  className="bg-gradient-to-r from-white via-white to-blue-200 h-full rounded-2xl shadow-[0_0_20px_rgba(255,255,255,0.6)] transition-all duration-1000"
                  style={{ width: "45%" }}
                ></div>
              </div>
              <p className="text-base text-blue-50 font-bold leading-relaxed opacity-90">
                You're earning points{" "}
                <span className="text-white font-black underline underline-offset-8 decoration-blue-300">
                  2x faster
                </span>{" "}
                than regular members. Keep it up!
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-6 pt-10 mt-6 border-t border-white/10 relative z-10">
            {[
              {
                label: "Total Points",
                val: loyaltyInfo?.totalPoints || 850,
              ...
              <div
                key={idx}
                className="bg-white/10 backdrop-blur-md rounded-3xl p-5 border border-white/5 hover:bg-white/20 transition-all duration-300"
              >
              ...
              <div className="bg-slate-50 rounded-[36px] p-8 space-y-8 border border-slate-100 shadow-inner">
                ...
                <div className="space-y-4 pt-2">
                  <div className="flex justify-between items-center text-sm font-bold">
                    <span className="text-[#94a3b8] uppercase tracking-widest text-[10px] font-black">
                      Time Slot
                    </span>
                    <span className="text-[#0f172a] bg-white px-3 py-1.5 rounded-xl shadow-sm border border-slate-100">
                  ...
                  <div className="flex justify-between items-center text-sm font-bold">
                    ...
                    <span className="px-3 py-1 bg-emerald-100 text-emerald-700 text-[10px] font-black rounded-lg uppercase">
                      Confirmed
                    </span>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {quickActions.map((action, i) => (
            ...
            >
              ...
              <div
                className={`w-20 h-20 ${action.bg} rounded-[28px] flex items-center justify-center shrink-0 group-hover:scale-110 group-hover:bg-blue-600 group-hover:text-white transition-all duration-500 shadow-sm relative z-10 group-hover:rotate-6`}
              >
                {React.cloneElement(
              ...
              <div className="relative z-10">
                <h4 className="font-black text-[#0f172a] text-lg group-hover:text-blue-600 transition-colors tracking-tight">
                  {action.name}
                </h4>
                <p className="text-xs text-[#94a3b8] font-bold uppercase tracking-widest mt-1.5 opacity-80">
                  {action.desc}
                </p>
              </div>
            </div>
