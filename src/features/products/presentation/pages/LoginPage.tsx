...
export const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        t("auth.login.toastSuccess", { defaultValue: "Đăng nhập thành công!" }),
      );
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (authData && authData.accessToken) {
          ...
          alert(
            t("auth.login.toastGoogleSuccess", {
              defaultValue: "Đăng nhập bằng Google thành công!",
            }),
          );
          navigate("/dashboard");
        } else {
          alert(
            t("auth.login.toastGoogleBackendFail", {
              defaultValue: "Không nhận được access token từ backend.",
            }),
          );
        }
      } else {
        alert(
          t("auth.login.toastGoogleBackendFail", {
            defaultValue: "Backend xác thực Google code thất bại.",
          }),
        );
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen w-full bg-[hsl(210,92%,91%)] dark:bg-[#0B0C10] flex flex-col p-4 md:p-6 antialiased font-sans overflow-y-auto relative transition-colors duration-300">
      ...
      <div className="fixed inset-0 pointer-events-none z-0 dark:hidden">
        <div
          className="absolute -top-32 -left-32 w-[600px] h-[600px] rounded-full bg-gradient-to-br from-sky-400/80 via-blue-200/40 to-transparent blur-3xl animate-pulse"
          style={{ animationDuration: "9s" }}
        />
        <div
          className="absolute -bottom-32 -right-20 w-[550px] h-[550px] rounded-full bg-gradient-to-tl from-cyan-400/70 via-sky-200/30 to-transparent blur-3xl animate-pulse"
      ...
      <div
        className="hidden dark:block absolute bottom-[-10%] right-[-10%] w-[50vw] h-[50vw] bg-purple-600/10 rounded-full blur-[120px] pointer-events-none mix-blend-lighten animate-pulse"
            ...
            <div className="w-full max-w-md mx-auto">
              ...
              <form onSubmit={handleSubmit} noValidate className="space-y-5">
                ...
                <div className="text-center text-sm font-medium text-slate-500 dark:text-slate-400 pt-4">
                  ...
                  <Link
                    to="/register"
                    className="text-blue-600 dark:text-blue-400 hover:text-blue-700 font-extrabold underline-offset-4 hover:underline transition-all"
                  >
              ...
              <div className="mt-8 pt-8 border-t border-slate-100 dark:border-white/5 text-center">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {["customer", "staff", "admin"].map((role) => (
                    ...
                    >
                      {t(
                        `auth.login.quickAccess${role.charAt(0).toUpperCase() + role.slice(1)}` as any,
                      )}
                    </button>
                  ))}
                </div>
