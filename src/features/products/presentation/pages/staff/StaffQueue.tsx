...
export const StaffQueuePage: React.FC = () => {
  ...
  const handleCashCheckout = async () => {
    ...
    try {
      ...
      toast.success(
        "Thanh toán tiền mặt thành công! Điểm thưởng đã được cộng.",
      );
  ...
  const columns = [
    ...
    },
    {
      ...
      buttonLabel: (status: string) =>
        status === "Queued" ? "Start Washing" : "Finish Wash",
      action: (status: string): StaffAction =>
        status === "Queued" ? "start" : "finish",
    },
    {
      title: "Completed",
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 font-bold">
            Manage vehicle workflow in real-time
          </p>
        </div>
        <div className="flex items-center gap-3 bg-white p-2 rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex -space-x-2">
            {[1, 2].map((i) => (
              <div
                key={i}
                className="w-8 h-8 rounded-full border-2 border-white bg-blue-100 flex items-center justify-center text-[10px] font-black text-blue-600"
              >
                S{i}
              </div>
            ))}
          </div>
          <span className="text-xs font-bold text-gray-400 uppercase tracking-widest pr-2">
            2 Staff Active
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 h-[calc(100vh-200px)] min-h-[600px]">
        {columns.map((col) => {
          const columnBookings = bookings.filter((b) =>
            col.status.includes(b.status),
          );

          return (
            <div
              key={col.title}
              className={`${col.bg} rounded-[32px] p-6 border-2 ${col.border} flex flex-col`}
            >
                  ...
                  columnBookings.map((booking) => {
                    const action =
                      typeof col.action === "function"
                        ? col.action(booking.status)
                        : col.action;
                    const label =
                      typeof col.buttonLabel === "function"
                        ? col.buttonLabel(booking.status)
                        : col.buttonLabel;
                      ...
                      >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-gray-400 font-bold uppercase">
                                {booking.washPackageId || "Standard Wash"}
                              </p>
                            </div>
                          </div>
                        ...
                        <button
                          ...
                          className={`w-full mt-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all active:scale-95 ${
                            action === "checkIn"
                              ? "bg-slate-800 text-white hover:bg-black shadow-slate-200"
                              : action === "queue"
