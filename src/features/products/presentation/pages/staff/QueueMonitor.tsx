...
export const QueueMonitor: React.FC = () => {
  ...
  const { startBooking, queueBooking, completedBooking } = useBooking({ loadMyBookings: false });
          ...
          <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-5 custom-scrollbar">
            {isWashBaysLoading ? (
              ...
              <div
                ...
                className={`rounded-2xl p-5 transition-all flex flex-col gap-4 border-2 cursor-pointer ${bay.isMaintenance
              ...
              >
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-sm ${bay.isMaintenance ? 'bg-rose-100 text-rose-600' :
                      ...
                      {bay.supportedTypes && bay.supportedTypes.length > 0 && (
                        <div className="flex gap-1.5 mt-1.5 flex-wrap">
                          {bay.supportedTypes.map((type: string) => (
                            <span key={type} className={`px-2 py-0.5 rounded text-[10px] font-bold tracking-wide uppercase ${bay.isMaintenance ? 'bg-rose-100/50 text-rose-600' :
                  ...
                  <span className={`px-3 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase shrink-0 ${bay.isMaintenance ? 'bg-rose-100 text-rose-700' :
