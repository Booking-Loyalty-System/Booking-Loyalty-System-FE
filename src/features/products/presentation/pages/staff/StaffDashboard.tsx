...
export const StaffDashboard: React.FC = () => {
    ...
    const { confirmBooking, checkInBooking, checkoutBooking, staffCancelBooking, scanQr, noShowBooking } = useBooking({ loadMyBookings: false });
    ...
    const filteredBookings = bookings.filter(b => {
        ...
        const matchesSearch = bookingCode.includes(search) || vehicleName.includes(search) || licensePlate.includes(search);
    ...
    const handleConfirmPayOS = async (): Promise<string> => {
        ...
        try {
            ...
            if (typeof response === 'object' && 'checkoutUrl' in response) return (response as any).checkoutUrl;
    ...
    useEffect(() => {
        ...
        if (paymentStatus) {
            ...
            const desc = isSuccess ? 'Giao dịch đã được xác nhận.' : 'Giao dịch link thanh toán đã bị hủy bỏ hoặc hết hạn.';
            if (isSuccess) toast.success(message, { description: desc, duration: 5000 });
    ...
    if (isStaffLoading) return (
        <div className="space-y-10 w-full animate-pulse">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[...Array(4)].map((_, i) => (
                    <div key={i} className="h-40 bg-slate-200 dark:bg-white/5 rounded-[2rem]"></div>
                ))}
            </div>
                    ...
                    <div className="inline-flex items-center gap-4 bg-white/80 dark:bg-[#111]/80 backdrop-blur-xl px-5 py-3 rounded-2xl border border-slate-200/50 dark:border-white/5 shadow-sm">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-medium text-slate-600 dark:text-slate-400">
                                Xin chào, <span className="font-extrabold text-slate-900 dark:text-white">{staffProfile.fullName}</span>
                            </span>
                        </div>
            ...
            <div className="bg-white/80 dark:bg-[#111]/80 backdrop-blur-2xl rounded-[2.5rem] border border-slate-200/60 dark:border-white/5 shadow-xl shadow-slate-200/20 dark:shadow-black/20 overflow-hidden flex flex-col transition-all duration-300">
                ...
                {isBookingsLoading ? (
                    <div className="p-32 flex flex-col items-center justify-center bg-transparent">
                        ...
                        <p className="text-sm font-bold text-slate-500 dark:text-slate-400 animate-pulse">Đang đồng bộ dữ liệu...</p>
                    </div>
                    ...
                    <div className="p-32 flex flex-col items-center justify-center text-slate-500 bg-transparent text-center space-y-3">
                        ...
                        <p className="text-xl font-extrabold text-slate-700 dark:text-slate-300">Không tìm thấy lịch đặt nào</p>
                        <p className="text-sm font-medium dark:text-slate-500">Thử thay đổi bộ lọc hoặc chọn ngày khác xem sao nhé.</p>
                    </div>
                ) : (
                    <div className="overflow-x-auto custom-scrollbar">
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-slate-50 dark:bg-white/5 border-b border-slate-200 dark:border-white/5">
                                <tr>
                                    <th className="py-5 px-6 text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest whitespace-nowrap">Mã Code</th>
                                    <th className="py-5 px-6 text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest whitespace-nowrap">Khách & Xe</th>
                                    <th className="py-5 px-6 text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest whitespace-nowrap">Dịch vụ</th>
                                    <th className="py-5 px-6 text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest whitespace-nowrap">Trạng thái</th>
                                    <th className="py-5 px-6 text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest whitespace-nowrap text-right">Thao tác</th>
                                </tr>
                            </thead>
                    ...
                    <div className="bg-white dark:bg-[#111] rounded-[2rem] shadow-2xl border border-slate-200 dark:border-white/10 w-full max-w-md overflow-hidden flex flex-col transform scale-100 animate-in zoom-in-95 duration-300">
                        <div className="p-6 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 flex justify-between items-center">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h2 className="font-extrabold text-xl text-slate-900 dark:text-white tracking-tight">Chi Tiết Lịch Đặt</h2>
                                    <p className="text-xs text-slate-500 font-bold mt-0.5">
                                        Mã: <span className="text-blue-600 dark:text-blue-400">{selectedBookingDetail.bookingCode}</span>
                                    </p>
                                </div>
                            </div>
                            <button onClick={() => setSelectedBookingDetail(null)} className="p-2.5 bg-white dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-white/10 text-slate-400 hover:text-slate-700 dark:hover:text-white rounded-xl transition-all shadow-sm border border-slate-200/50 dark:border-white/5">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        ...
                        <div className="p-6 space-y-6">
                            <div className="flex items-center justify-between p-5 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-500/10 dark:to-indigo-500/10 rounded-2xl border border-blue-100 dark:border-blue-500/20">
                                <div>
                                    <p className="text-[10px] font-black text-blue-500 dark:text-blue-400 uppercase tracking-widest mb-1">Biển số xe</p>
                                    <p className="text-2xl font-black text-slate-900 dark:text-white">{selectedBookingDetail.licensePlate}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-[10px] font-black text-blue-500 dark:text-blue-400 uppercase tracking-widest mb-1">Dòng xe</p>
                                    <p className="text-lg font-bold text-slate-700 dark:text-slate-300">{selectedBookingDetail.vehicleName}</p>
                                </div>
                            </div>

                            <div className="space-y-4">
                                {[
                                    { label: 'Dịch vụ:', value: selectedBookingDetail.serviceName, primary: true },
                                    { label: 'Khung giờ:', value: `${selectedBookingDetail.startTime} - ${selectedBookingDetail.bookingDate}` },
                                    { label: 'Trạng thái:', value: selectedBookingDetail.status, isStatus: true },
                                ].map((item, i) => (
                                    <div key={i} className="flex justify-between items-center py-3 border-b border-slate-100 dark:border-white/5 border-dashed">
                                        ...
                                        {item.isStatus ? (
                                            ...
                                            <span className={`text-sm font-bold text-right max-w-[60%] ${item.primary ? 'text-slate-900 dark:text-white' : 'text-slate-700 dark:text-slate-300'}`}>
                                                {item.value}
                                            </span>
                                        )}
                                    </div>
                        ...
                        <div className="p-5 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 flex gap-3">
                            <button onClick={() => setSelectedBookingDetail(null)} className="flex-1 py-3.5 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#111] text-slate-600 dark:text-slate-300 font-bold text-sm hover:bg-slate-50 dark:hover:bg-white/5 transition-all shadow-sm">
                                Đóng
                            </button>
                        </div>
                    </div>
