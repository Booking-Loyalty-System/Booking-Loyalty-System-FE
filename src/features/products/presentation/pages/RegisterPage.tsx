...
export const RegisterPage: React.FC = () => {
  ...
  // Thêm hook navigate để chuyển trang mượt mà
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Bắt buộc có await để catch lỗi API quăng ra
      await register({
        email,
        password,
        fullName,
      ...
      navigate("/dashboard"); // Chuyển trang mượt mà không reload
    } catch (error: any) {
      console.error("Đăng ký email thất bại:", error);
      // Xử lý báo lỗi 409 (Email tồn tại) hoặc lỗi chung
  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Có await để bắt lỗi nếu API server của bạn từ chối
      await registerWithPhone({
        phoneNumber: verifiedPhone,
        idToken: idToken,
      });
      toast.success("Xác thực OTP thành công! Đang đăng nhập...");
      navigate("/dashboard"); // Chuyển trang mượt mà
    } catch (error) {
      console.error("Sai OTP hoặc lỗi server:", error);
      toast.error("Mã OTP không chính xác hoặc đăng ký thất bại!");
    }
  };
          ...
          <div>
            ...
            <div className="bg-[#f0f6fe] rounded-2xl pt-4 pb-6 px-6">
              ...
              <ul className="space-y-3">
                <li className="flex items-center gap-3 text-base text-[#334155]">
                  <CheckCircle2 className="w-6 h-6 text-[#4a90e2] fill-current text-white" />
            ...
            {registerMode === "email" && (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={isPending}
                  className="w-full bg-[#4a90e2] text-white py-2.5 rounded-xl font-semibold hover:bg-[#357abd] disabled:opacity-50"
                >
