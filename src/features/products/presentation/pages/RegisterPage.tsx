...
export const RegisterPage: React.FC = () => {
  ...
  const isValidPhoneNumber = (value: string) => /^0\d{9}$/.test(value);
  ...
  const validateEmailRegisterForm = () => {
    ...
    if (!normalizedFullName) {
      errors.fullName = t("auth.validation.fullNameRequired", {
        defaultValue: "Họ và tên không được để trống.",
      });
    } else if (normalizedFullName.length > 100) {
      errors.fullName = t("auth.validation.fullNameMaxLength", {
        defaultValue: "Họ và tên không được vượt quá 100 ký tự.",
      });
    }

    if (!normalizedPhone) {
      errors.phoneNumber = t("auth.validation.phoneRequired", {
        defaultValue: "Số điện thoại không được để trống.",
      });
    } else if (!isValidPhoneNumber(normalizedPhone)) {
      errors.phoneNumber = t("auth.validation.phoneInvalid", {
        defaultValue:
    ...
    if (!normalizedEmail) {
      errors.email = t("auth.validation.emailRequired", {
        defaultValue: "Email không được để trống.",
      });
    } else if (!isValidEmail(normalizedEmail)) {
      errors.email = t("auth.validation.emailInvalid", {
        defaultValue: "Email không đúng định dạng.",
      });
    }

    if (!password) {
      errors.password = t("auth.validation.passwordRequired", {
        defaultValue: "Mật khẩu không được để trống.",
      });
    } else if (password.length < 6) {
      errors.password = t("auth.validation.passwordMinLength", {
        defaultValue: "Mật khẩu phải có ít nhất 6 ký tự.",
      });
    }

    if (!confirmPassword) {
      errors.confirmPassword = t("auth.validation.confirmPasswordRequired", {
        defaultValue: "Vui lòng nhập lại mật khẩu.",
      });
    } else if (password !== confirmPassword) {
      errors.confirmPassword = t("auth.register.toastPasswordMismatch", {
  ...
  const validatePhoneRegisterNumber = () => {
    const normalizedPhone = normalizePhoneNumber(phoneNumber);

    if (!normalizedPhone) {
      setFieldErrors((prev) => ({
        ...prev,
        phoneNumber: t("auth.validation.phoneRequired", {
          defaultValue: "Số điện thoại không được để trống.",
        }),
      }));
      return false;
    ...
    if (!isValidPhoneNumber(normalizedPhone)) {
      setFieldErrors((prev) => ({
        ...prev,
        phoneNumber: t("auth.validation.phoneInvalid", {
          defaultValue:
            "Số điện thoại phải gồm đúng 10 chữ số và bắt đầu bằng 0.",
        }),
      }));
    ...
    try {
      ...
      if (error?.response?.status === 409) {
        ...
        if (message.includes("phone")) {
          setFieldErrors((prev) => ({
            ...prev,
            phoneNumber: t("auth.register.toastPhoneExist", {
              defaultValue: "Số điện thoại này đã được sử dụng.",
            }),
          }));
        } else {
          setFieldErrors((prev) => ({
            ...prev,
            email: t("auth.register.toastEmailExist", {
              defaultValue: "Email này đã được đăng ký rồi.",
            }),
          }));
        }
      } else {
        toast.error(
          t("auth.register.toastFailed", {
            defaultValue: "Đăng ký thất bại, vui lòng thử lại.",
          }),
        );
      }
    }
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (!/^\d{6}$/.test(otpCode)) {
      setFieldErrors((prev) => ({
        ...prev,
        emailOtp: t("auth.register.toastOtpInvalid", {
          defaultValue: "OTP phải gồm đúng 6 chữ số.",
        }),
      }));
      return;
    }
    ...
    try {
      ...
      toast.success(
        t("auth.register.toastOtpSuccess", {
          defaultValue: "Xác thực email thành công! Chào mừng bạn!",
        }),
      );
      ...
      setFieldErrors((prev) => ({
        ...prev,
        emailOtp: t("auth.register.toastOtpInvalid", {
          defaultValue: "Mã OTP không chính xác hoặc đã hết hạn.",
        }),
      }));
    }
  };
  ...
  const handleSendOTP = async (e: React.MouseEvent) => {
    ...
    if (!validatePhoneRegisterNumber()) return;
    ...
    try {
      ...
      if (!appVerifier) {
        toast.error(
          t("auth.register.toastRecaptchaError", {
            defaultValue: "Không thể khởi tạo bộ xác thực reCaptcha.",
          }),
        );
        return;
      }
      ...
      setIsOtpSent(true);
      toast.success(
        t("auth.register.toastOtpSent", {
          defaultValue: "Đã gửi mã OTP thành công!",
        }),
      );
    } catch (error) {
      ...
      toast.error(
        t("auth.register.toastOtpSendFail", {
          defaultValue: "Không thể gửi OTP: ",
        }) + err.message,
      );
    }
  };
  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    ...
    if (!/^\d{6}$/.test(otp)) {
      setFieldErrors((prev) => ({
        ...prev,
        phoneOtp: t("auth.register.toastOtpInvalid", {
          defaultValue: "OTP phải gồm đúng 6 chữ số.",
        }),
      }));
      return;
    ...
    if (!confirmResult) {
      toast.warning(
        t("auth.register.toastSendOtpFirst", {
          defaultValue: "Vui lòng gửi mã OTP trước.",
        }),
      );
      return;
    }
    ...
    try {
      ...
      toast.success(
        t("auth.register.toastOtpSuccess", {
          defaultValue: "Xác thực OTP thành công! Đang đăng nhập...",
        }),
      );
      navigate("/dashboard");
    } catch (error) {
      ...
      setFieldErrors((prev) => ({
        ...prev,
        phoneOtp: t("auth.register.toastOtpInvalid", {
          defaultValue: "Mã OTP không chính xác hoặc đăng ký thất bại.",
        }),
      }));
    }
  };
  ...
  return (
    <div className="min-h-screen w-full bg-[hsl(210,92%,91%)] dark:bg-[#0B0C10] flex flex-col p-4 md:p-6 antialiased font-sans overflow-y-auto relative transition-colors duration-300">
      ...
      <div className="fixed inset-0 pointer-events-none z-0 dark:hidden">
        <div
          className="absolute -top-32 -right-32 w-[600px] h-[600px] rounded-full bg-gradient-to-bl from-sky-400/80 via-blue-200/40 to-transparent blur-3xl animate-pulse"
          style={{ animationDuration: "10s" }}
        />
        <div
          className="absolute -bottom-32 -left-20 w-[550px] h-[550px] rounded-full bg-gradient-to-tr from-cyan-400/70 via-sky-200/30 to-transparent blur-3xl animate-pulse"
      ...
      <div
        className="hidden dark:block absolute bottom-[-10%] left-[-10%] w-[50vw] h-[50vw] bg-indigo-600/10 rounded-full blur-[120px] pointer-events-none mix-blend-lighten animate-pulse"
                ...
                <ul className="space-y-4">
                  {[
                    ...
                    <li key={idx} className="flex items-start gap-4">
                      ...
                      <span className="text-base font-semibold text-slate-700 dark:text-slate-300 pt-0.5">
                        {benefit}
                      </span>
                    </li>
                  ))}
                ...
                >
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                      <div className="relative flex-1 group">
                        ...
                        <input
                          ...
                          className={`w-full pl-11 pr-4 py-3.5 bg-slate-50 dark:bg-white/5 border rounded-2xl text-base text-blue-950 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-4 transition-all font-medium disabled:opacity-60 ${
                  ...
                  {isOtpSent && (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                      <InputField
                        icon={<KeyRound />}
                        label={t("auth.register.labelOtp", {
                          defaultValue: "Mã OTP",
                        })}
                        value={otp}
                        onChange={(value) => {
                        ...
                        placeholder={t("auth.register.placeholderOtp", {
                          defaultValue: "Nhập 6 số OTP...",
                        })}
                        type="text"
                        inputMode="numeric"
                        maxLength={6}
                      ...
                      >
                        {isPendingPhone ? (
                          ...
                          <>
                            {t("auth.register.confirmAndRegister", {
                              defaultValue: "Xác nhận & Đăng ký",
                            })}
                            <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                          </>
                        )}
          ...
          <div className="relative w-full max-w-md bg-white dark:bg-[#13151A] rounded-[2rem] p-8 shadow-2xl border border-slate-200/50 dark:border-white/10 animate-in fade-in zoom-in-95 duration-300">
            ...
            <div className="flex flex-col items-center text-center mb-8">
              ...
              <h3 className="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-sky-500 dark:text-white mb-2">
                {t("auth.register.otpModalTitle", {
                  defaultValue: "Xác thực Email",
                })}
              </h3>
              <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed">
                {t("auth.register.otpModalDesc", {
                  defaultValue: "Chúng tôi đã gửi mã 6 chữ số đến",
                })}
              </p>
            ...
            <form onSubmit={handleVerifyOtp}>
              ...
              {fieldErrors.emailOtp && (
                <p className="-mt-4 mb-6 text-center text-sm font-semibold text-rose-500 dark:text-rose-400">
                  {fieldErrors.emailOtp}
                </p>
              )}
                ...
                {isPendingVerify ? (
                  <>
                    ...
                    {t("auth.register.verifying", {
                      defaultValue: "Đang xác thực...",
                    })}
                  </>
                ) : (
                  <>
                    <ShieldCheck className="w-5 h-5" />
                    {t("auth.register.btnVerify", {
                      defaultValue: "Xác nhận OTP",
                    })}
                  </>
                )}
              </button>
            ...
            <div className="mt-6 text-center space-y-2">
              <p className="text-xs text-slate-400 dark:text-slate-500">
                {t("auth.register.otpExpiry", {
                  defaultValue: "Mã OTP có hiệu lực trong 10 phút",
                })}
              </p>
              <button
                type="button"
              ...
              >
                <X className="w-3 h-3" />
                {t("auth.register.cancelVerify", {
                  defaultValue: "Hủy và đăng ký lại",
                })}
              </button>
            </div>
          </div>
