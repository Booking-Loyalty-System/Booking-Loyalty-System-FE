...
export const BookingDetailModal: React.FC<Props> = ({ booking, onClose }) => {
  const { downloadInvoice, isDownloadingInvoice } = useBooking({
  ...
  const isComplex =
    hasFeedback || (booking.images && booking.images.length > 0) || isStaff;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
      <div
        className={`bg-white rounded-2xl p-5 md:p-6 w-full shadow-2xl relative transition-all duration-300 max-h-[95vh] overflow-y-auto ${isComplex ? "max-w-2xl" : "max-w-sm"}`}
      >
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 z-10 bg-white rounded-full p-1 shadow-sm border border-slate-100"
        >
          <X size={20} />
        </button>
        ...
        <div className={isComplex ? "md:grid md:grid-cols-5 md:gap-8" : ""}>
          ...
          <div
            className={`${isComplex ? "md:col-span-2 md:border-r md:border-slate-100 md:pr-8" : ""}`}
          >
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-slate-400 flex items-center gap-1.5">
                  <ShieldCheck size={16} /> Gói
                </span>
                <span
                  className="font-bold text-blue-950 text-right max-w-[150px] truncate"
                  title={booking.washPackageName}
                >
                  {booking.washPackageName}
                </span>
              </div>
            </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1 text-amber-500 bg-white px-2 py-1 rounded-full border border-amber-100 shadow-sm">
                        ...
                        <span className="text-xs font-black">
                          {booking.feedbackResponse.overallRating}/5
                        </span>
                      </div>
                    </div>
