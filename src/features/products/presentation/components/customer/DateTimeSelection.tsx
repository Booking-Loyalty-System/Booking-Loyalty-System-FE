...
export const DateTimeSelection: React.FC<DateTimeSelectionProps> = ({
  ...
  const handleCollapse = () => {
    ...
    const selectedIndex = dynamicDateSlots.findIndex(
  ...
  const currentDayData = weeklySummary.find(
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <h3 className="text-xl font-bold text-[#0f172a] dark:text-white">
          {t("bookWash.dateTime.selectDateTitle", {
        ...
        <div className="grid grid-cols-4 sm:grid-cols-7 gap-3">
          {visibleSlots.map((slot) => {
            ...
            const translatedDayName = t(
              `common.days.${slot.dayName.toLowerCase()}`,
              { defaultValue: slot.dayName },
            );
            return (
              <div
                key={slot.apiDate}
              ...
              >
                <span
                  className={`block text-xs font-semibold uppercase ${isDateSelected ? "text-[#1e6ffd] dark:text-blue-400" : "text-[#64748b] dark:text-slate-400"}`}
                >
                  {translatedDayName}
                </span>
                <span
                  className={`block text-2xl font-black mt-2 ${isDateSelected ? "text-[#1e6ffd] dark:text-blue-400" : "text-[#0f172a] dark:text-white"}`}
                >
                  {slot.dayNum}
                </span>
              </div>
        ...
        {(hasMoreDays || canCollapse) && (
          <div className="flex justify-center gap-4 mt-2">
            {hasMoreDays && (
              ...
              >
                <span>
                  {t("bookWash.dateTime.showMore", {
                    defaultValue: "Xem tiếp 7 ngày tới",
                  })}
                </span>
      ...
      <div className="space-y-4 pt-2">
        <h4 className="text-lg font-bold text-[#0f172a] dark:text-white">
          {t("bookWash.dateTime.availableTimeSlotsTitle", {
            defaultValue: "Available Time Slots",
          })}
        </h4>
        ...
        {isLoadingSlots ? (
          <div className="text-sm font-medium text-blue-500 py-4 flex items-center gap-2">
            ...
            {t("bookWash.dateTime.loadingSlots", {
              defaultValue: "Đang tải dữ liệu ca trống...",
            })}
          </div>
        ) : activeTimeSlots.length === 0 ? (
          <div className="text-sm font-medium text-slate-400 py-4 italic">
            {t("bookWash.dateTime.selectBranchPrompt", {
              defaultValue: "Vui lòng chọn chi nhánh để cập nhật ca trống...",
            })}
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {activeTimeSlots.map((slot) => {
              ...
              const isFull =
                !slot.isAvailable ||
                slot.slotRatio.toLowerCase().includes("full");
              ...
              if (isDisabled) {
                statusClasses =
                  "border-[#e2e8f0] dark:border-white/5 bg-slate-50 dark:bg-white/5 text-slate-400 dark:text-slate-500 cursor-not-allowed opacity-60";
              } else if (isTimeSelected) {
                statusClasses =
                  "border-[#1e6ffd] dark:border-blue-500 ring-2 ring-blue-100 dark:ring-blue-900/30 bg-white dark:bg-[#13151A] text-[#1e6ffd] dark:text-blue-400";
              } else if (
                slot.slotRatio.includes("1") ||
                slot.slotRatio.includes("2")
              ) {
                // Nếu còn ít khoang rảnh (1 hoặc 2), chuyển box sang cảnh báo màu cam (Limited)
                statusClasses =
                  "border-[#fbd38d] dark:border-amber-500/30 bg-[#fffaf0] dark:bg-amber-500/10 text-amber-700 dark:text-amber-400";
              } else {
                // Còn nhiều khoang rảnh rỗi (Màu xanh lá nhẹ dịu)
                statusClasses =
                  "border-[#bbf7d0] dark:border-emerald-500/30 bg-[#f0fdf4] dark:bg-emerald-500/10 text-emerald-800 dark:text-emerald-400";
              }
              ...
              return (
                ...
                >
                  ...
                  <span className="text-xs font-semibold pl-6">
                    {isDisabled
                      ? isPast
                        ? t("bookWash.dateTime.closed", {
                            defaultValue: "Closed",
                          })
                        : t("bookWash.dateTime.full", { defaultValue: "Full" })
                      : i18n.language === "en"
                        ? slot.slotRatio
                            .replace(/Khoang trống/gi, "Available")
                            .replace(/Khoang/gi, "Slot")
                            .replace(/Đã đầy|Kín chỗ/gi, "Full")
                        : slot.slotRatio}
                  </span>
                </div>
              );
