    ...
    <div className="space-y-10 max-w-7xl mx-auto pb-12 animate-fade-in text-slate-800 dark:text-slate-100">
      ...
      <div className="space-y-2">
        <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white flex items-center gap-3">
          ...
          <span className="animate-bounce inline-block origin-bottom-right">👋</span>
        </h1>
        ...
        {stats.map((stat) => (
          ...
          >
            ...
            <div className="relative z-10 flex items-center justify-between">
              ...
              <div className={`w-12 h-12 ${stat.bg} rounded-2xl flex items-center justify-center border border-white/50 dark:border-white/5 shadow-sm group-hover:scale-110 transition-transform duration-300`}>
                {stat.icon}
              </div>
            </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="text-left md:text-right">
              <span className="block text-4xl font-black text-emerald-600 dark:text-emerald-400 tracking-tighter">
                {currentCycleWashes}<span className="text-2xl text-emerald-400 dark:text-emerald-700">/7</span>
              </span>
        ...
        <div className="lg:col-span-2 bg-gradient-to-br from-slate-900 to-[#111] dark:from-[#050505] dark:to-[#111] border border-slate-800/50 dark:border-white/10 rounded-[2.5rem] p-8 lg:p-10 text-white relative overflow-hidden shadow-2xl min-h-[380px] flex flex-col justify-between group">
          ...
          <div className="space-y-8 relative z-10">
            ...
            <div className="space-y-3 bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-3xl">
              ...
              <p className="text-sm font-medium text-slate-400 pt-1">
                <span className="text-white font-bold">{Math.max(0, 1000 - totalPoints)}</span> {t('dashboard.pointsToNextTier', 'points to next tier')}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 pt-6 mt-6 relative z-10">
            <div className="bg-white/5 hover:bg-white/10 transition-colors backdrop-blur-sm border border-white/10 rounded-2xl p-4 lg:p-5">
              <span className="block text-2xl lg:text-3xl font-black text-white">{totalPoints}</span>
            ...
            <div className="bg-white/5 hover:bg-white/10 transition-colors backdrop-blur-sm border border-white/10 rounded-2xl p-4 lg:p-5">
              <span className="block text-2xl lg:text-3xl font-black text-amber-400 drop-shadow-[0_0_10px_rgba(251,191,36,0.3)]">
                {tier === "Gold" ? "2x" : tier === "Platinum" ? "3x" : tier === "Silver" ? "1.5x" : "1x"}
              </span>
              <span className="text-[10px] lg:text-xs font-bold text-slate-400 uppercase tracking-widest mt-1 block">
            ...
            <div className="bg-white/5 hover:bg-white/10 transition-colors backdrop-blur-sm border border-white/10 rounded-2xl p-4 lg:p-5">
              <span className="block text-2xl lg:text-3xl font-black text-blue-400 drop-shadow-[0_0_10px_rgba(96,165,250,0.3)]">
                {tier === "Platinum" ? 14 : tier === "Gold" ? 12 : tier === "Silver" ? 10 : 7}
              </span>
              <span className="text-[10px] lg:text-xs font-bold text-slate-400 uppercase tracking-widest mt-1 block">
              ...
              <div className="bg-gradient-to-br from-slate-50 to-white dark:from-white/5 dark:to-transparent border border-slate-200/50 dark:border-white/10 rounded-3xl p-6 space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500 font-bold mt-0.5">
                      {new Date(nextBooking.bookingDate).toLocaleDateString("vi-VN", { weekday: 'long', year: 'numeric', month: 'short', day: 'numeric' })}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4 text-sm font-semibold pt-2">
                  <div className="flex justify-between items-center bg-white dark:bg-black/20 p-3 rounded-2xl border border-slate-100 dark:border-white/5">
                    <span className="text-slate-500">{t('dashboard.bookingTime', 'Time')}</span>
                    <span className="text-slate-900 dark:text-white text-lg font-black">{nextBooking.startTime}</span>
                  </div>
                  <div className="flex justify-between items-center px-2">
                    <span className="text-slate-500">{t('dashboard.bookingVehicle', 'Vehicle')}</span>
                    <span className="text-slate-900 dark:text-white uppercase font-bold tracking-wider bg-slate-100 dark:bg-white/10 px-3 py-1 rounded-lg">
                      {(nextBooking as any).licensePlate || (nextBooking as any).vehiclePlate || "N/A"}
                    </span>
                  </div>
                  <div className="flex justify-between items-center px-2">
                    <span className="text-slate-500">{t('dashboard.bookingStatus', 'Status')}</span>
                    <span
      ...
      <div className="space-y-6 pt-4">
        <h3 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">{t('dashboard.quickActionsTitle', 'Quick Actions')}</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {quickActions.map((action, i) => (
            ...
            >
              <div className={`w-14 h-14 bg-gradient-to-br ${action.gradient} rounded-2xl flex items-center justify-center shrink-0 shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                {action.icon}
              </div>
              <div>
