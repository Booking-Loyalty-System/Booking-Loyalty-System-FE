...
import {
    Car, Plus, Info, Droplet, Sparkles, ShieldAlert, ChevronLeft, ChevronRight, Clock
} from 'lucide-react';
import { useVehicle } from "@/features/products/application/useVehicle.ts";
import { useWashPackage } from "@/features/products/application/useWashPackage.ts"; // 🌟 Import hook mới tạo
...
export const BookWash: React.FC = () => {
    ...
    const [selectedDate, setSelectedDate] = useState<string>(dynamicDateSlots[0].fullDate);
            ...
            <div className="flex-1 space-y-10 w-full">
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-[#64748b] font-medium">{vehicle.type}</p>
                                        </div>
                                    </div>
                        ...
                        <div onClick={handleAddNewVehicle} className="cursor-pointer border-2 border-dashed border-[#cbd5e1] hover:border-[#1e6ffd] bg-white hover:bg-blue-50/10 rounded-2xl p-5 transition-all duration-200 flex flex-col items-center justify-center min-h-[142px] group">
                        ...
                        <div className="grid grid-cols-4 sm:grid-cols-7 gap-3">
                            {dynamicDateSlots.map((slot) => {
                                ...
                                return (
                                    ...
                                    >
                                        <span className={`block text-xs font-semibold uppercase tracking-wider ${isDateSelected ? 'text-[#1e6ffd]' : 'text-[#64748b]'}`}>
                                            {slot.dayName}
                                        </span>
                    ...
                    <div className="space-y-4 pt-2">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                            {timeSlots.map((slot, index) => {
                                ...
                                let statusClasses = 'border-[#e2e8f0] bg-white hover:border-[#cbd5e1]';
                                if (isFull) {
                                    statusClasses = 'border-[#e2e8f0] bg-slate-50/70 opacity-60 cursor-not-allowed';
                                } else if (isLimited) {
                                    statusClasses = 'border-[#fbd38d] bg-[#fffaf0] hover:border-orange-400';
                                } else if (slot.status === 'available') {
                                    statusClasses = 'border-[#bbf7d0] bg-[#f0fdf4] hover:border-emerald-400';
                                }
                                ...
                                return (
                                    ...
                                    >
                                        {(isFull || isLimited) && (
                                            <span className={`absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full flex items-center justify-center text-[10px] text-white shadow-sm font-bold ${isFull ? 'bg-slate-400' : 'bg-orange-400'}`}>
                                                ⚡
                                            </span>
                                        )}

                                        <div className="flex items-center gap-2">
                                            <Clock className={`w-4 h-4 ${isTimeSelected ? 'text-[#1e6ffd]' : isFull ? 'text-slate-400' : isLimited ? 'text-orange-600' : 'text-emerald-600'}`} />
                                            <span className={`text-sm font-black ${isTimeSelected ? 'text-[#1e6ffd]' : isFull ? 'text-slate-500' : isLimited ? 'text-orange-700' : 'text-emerald-700'}`}>
                                                {slot.time}
                                            </span>
                                        </div>

                                        <span className={`text-xs font-semibold pl-6 ${isFull ? 'text-slate-400' : isLimited ? 'text-orange-600' : 'text-emerald-600'}`}>
                            ...
                            <div className="space-y-4 text-sm">
                                ...
                                <div className="flex flex-col gap-1 border-t border-[#f1f5f9] pt-3">
                                    ...
                                    <span className="font-bold text-[#334155]">
                                        {selectedDate} {selectedTime ? `@ ${selectedTime}` : '(Please choose time)'}
                                    </span>
                                </div>
