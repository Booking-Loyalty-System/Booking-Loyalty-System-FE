...
export const RewardsSection: React.FC = () => {
    ...
    const { redemptions, redeemReward, isRedeeming, availableRewards, isLoadingRewards } = useReward();
    ...
    const rewards: RewardItem[] = useMemo(() => {
        ...
        return availableRewards.map(reward => {
            ...
            return {
                ...
                description: reward.description ?? 'Đổi điểm để nhận ưu đãi giảm giá.',
    ...
    const redeemableCount = useMemo(() => {
        return rewards.filter(r => !r.comingSoon && availablePoints >= r.requiredPts).length;
    ...
    const handleRedeemClick = async (rewardId: string, cost: number, title: string) => {
        ...
        try {
            ...
            toast.success(`Đổi thành công: ${title}! Voucher đã được thêm vào tài khoản của bạn.`);
        } catch (err) {
        ...
        <div className="w-full space-y-8 text-slate-800">
            ...
            <div className="bg-emerald-50/60 border border-emerald-100 rounded-2xl p-5 flex items-start gap-4">
                ...
                <div className="space-y-1">
                    <h4 className="font-bold text-emerald-900 text-base">Tự động áp dụng khi đặt lịch</h4>
                    <p className="text-sm text-emerald-800 leading-relaxed">
                        Tất cả voucher sau khi đổi sẽ được <strong className="text-emerald-900 font-extrabold">tự động tối ưu tại trang thanh toán</strong>. Bạn không cần phải nhập mã thủ công!
                    </p>
                </div>
            </div>
            ...
            <div>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-6">
                    <h2 className="text-2xl font-extrabold text-slate-800 tracking-tight">Phần thưởng khả dụng</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {isLoadingRewards ? (
                        ...
                        return (
                            <div
                                key={item.id}
                                className={`bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between relative ${item.comingSoon ? 'opacity-75' : ''
                                    }`}
                            >
                                {item.comingSoon && (
                                ...
                                <div className="mt-6 pt-5 border-t border-slate-100 flex items-center justify-between gap-4">
                                    <div>
                                        <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">Yêu cầu</p>
                                        <p className="text-xl font-black text-slate-800 mt-0.5">{item.requiredPts} <span className="text-sm font-bold text-slate-500">điểm</span></p>
                                    </div>
                                    ...
                                    <button
                                        ...
                                        className={`font-bold text-sm px-6 py-2.5 rounded-xl shadow-sm transition-all ${!item.comingSoon && canAfford
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <div className="p-6 border-b border-slate-100 flex items-center gap-2.5">
                    ...
                    <h3 className="text-lg font-bold text-slate-800 tracking-tight">Lịch sử đổi thưởng</h3>
                </div>

                <div className="divide-y divide-slate-100">
                    {!Array.isArray(redemptions) || redemptions.length === 0 ? (
                        <p className="text-sm text-slate-400 font-medium text-center py-8">Bạn chưa đổi phần thưởng nào.</p>
                    ) : redemptions.map((v) => {
                        if (!v) return null;
                        return (
                            <div
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-slate-800 text-base tracking-tight">{v.rewardName ?? 'Voucher giảm giá'}</h4>
                                        <p className="text-sm text-slate-400 font-medium mt-1">Hạn dùng: Khả dụng · Mã: <span className="font-mono">REDEEM-{v.pointsSpent}PTS</span></p>
                                    </div>
                                </div>
                                ...
                                <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2 border-t sm:border-t-0 pt-3 sm:pt-0 border-slate-100">
                                    <p className="text-lg font-black text-slate-800 tracking-tight">
                                        -{v.pointsSpent ?? 0} <span className="text-xs font-bold text-slate-500">điểm</span>
                                    </p>
                                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${v.status === 'Active'
