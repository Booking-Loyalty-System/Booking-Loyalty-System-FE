...
export const LoyaltyTier: React.FC = () => {
  ...
  const userData = customerMe;

  // Lấy dữ liệu thật từ userData, nếu chưa có thì fallback
  const currentPoints = userData?.totalPoints || 0;
  const currentTierName = userData?.tier || "Bronze";
  ...
  const baseTiers: MembershipTier[] = [
    {
      name: "Bronze", // Sửa từ Member thành Bronze để khớp API
  ...
  // Lấy dữ liệu thật từ userData cho các thống kê tổng quan
  const totalEarned = userData?.totalPoints || historyData?.totalEarnedThisMonth || 0;
  const totalRedeemed = historyData?.totalRedeemedThisMonth || 0;
  const totalBookings = userData?.totalWashes || historyData?.totalBookingsThisMonth || 0;
