...
export const VehicleSelection: React.FC<VehicleSelectionProps> = ({
    ...
    return (
        <div className="space-y-4">
            <h3 className="text-xl font-bold text-[#0f172a]">{t('bookWash.vehicle.selectTitle', { defaultValue: "Select Vehicle" })}</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="grid grid-cols-2 text-sm pt-3 border-t border-[#f1f5f9]">
                                <div>
                                    <span className="block text-xs text-[#94a3b8] font-medium">{t('bookWash.vehicle.color', { defaultValue: "Color" })}</span>
                                    <span className="font-semibold text-[#1e293b]">{vehicle.color}</span>
                                </div>
                                <div>
                                    <span className="block text-xs text-[#94a3b8] font-medium">{t('bookWash.vehicle.plate', { defaultValue: "Plate" })}</span>
                ...
                <div onClick={onAddNewVehicle} className="cursor-pointer border-2 border-dashed border-[#cbd5e1] hover:border-[#1e6ffd] bg-white hover:bg-blue-50/10 rounded-2xl p-5 transition-all duration-200 flex flex-col items-center justify-center min-h-[142px] group">
                    ...
                    <span className="text-sm font-bold text-slate-500 group-hover:text-[#1e6ffd]">{t('bookWash.vehicle.addNew', { defaultValue: "Add New Vehicle" })}</span>
                </div>
            </div>
        </div>
    );
