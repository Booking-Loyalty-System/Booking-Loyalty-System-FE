...
import { 
    ...
    CheckCircle2, Info, ShieldCheck, Zap
} from 'lucide-react';
...
export const PaymentStep: React.FC<PaymentStepProps> = ({ 
    ...
    const handlePointsChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const val = Math.min(Number(e.target.value), availablePoints, totalAmount * 10);
        setPointsToUse(val);
    ...
    return (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="text-center space-y-2">
                ...
                <p className="text-slate-500 font-medium">Select how you'd like to pay for your service</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    { id: 'card', label: 'Credit Card', icon: CreditCard, desc: 'Visa, Mastercard', color: 'blue' },
                    { id: 'qr', label: 'QR Pay', icon: Smartphone, desc: 'Scan to pay', color: 'purple' },
                    { id: 'cash', label: 'Pay at Counter', icon: Wallet, desc: 'Cash or POS', color: 'emerald' },
                ].map((method) => {
                    ...
                    return (
                        ...
                        >
                            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all duration-300 ${
                                isActive ? 'bg-blue-600 text-white' : 'bg-slate-50 text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-600'
                            }`}>
                                <Icon className="w-8 h-8" />
                            </div>
                            <div>
                                <h4 className={`font-black tracking-tight ${isActive ? 'text-blue-600' : 'text-slate-900'}`}>{method.label}</h4>
                                <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">{method.desc}</p>
                            </div>
                            {isActive && (
            ...
            <div className="bg-slate-900 rounded-[40px] p-8 text-white relative overflow-hidden group">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <h3 className="text-xl font-black tracking-tight">Use Loyalty Points</h3>
                            <p className="text-slate-400 text-sm font-medium mt-1">You have <span className="text-white font-black">{availablePoints} points</span> available.</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        {usePoints && (
                            <div className="flex flex-col items-end">
                                ...
                                />
                                <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mt-1">-${pointValue.toFixed(2)} Off</span>
                            </div>
                        )}
                        <button 
                            onClick={() => togglePoints(!usePoints)}
                            className={`px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${
                                usePoints ? 'bg-rose-500 text-white' : 'bg-white text-slate-900 hover:bg-blue-50'
                            }`}
                        >
            ...
            <div className="bg-white border-2 border-slate-100 rounded-[40px] p-10 flex flex-col md:flex-row items-center justify-between gap-8">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        <h4 className="text-lg font-black text-slate-900 tracking-tight">Secure Payment</h4>
                        <p className="text-sm text-slate-500 font-medium">Your transaction is encrypted and safe.</p>
                    </div>
                </div>

                <div className="flex items-center gap-10">
                    <div className="text-right">
                        <p className="text-xs text-slate-400 font-black uppercase tracking-[0.2em]">Total to Pay</p>
            ...
            <div className="flex items-center gap-3 justify-center text-slate-400">
                ...
                <p className="text-xs font-bold uppercase tracking-widest">Pricing includes all applicable taxes and fees</p>
            </div>
        </div>
    );
};
