...
import type { Vehicle, VehicleFormData } from '@/features/products/domain/models/vehicle/vehicle.model';
...
export const VehicleStep: React.FC<VehicleStepProps> = ({ onSelect, selectedVehicleId }) => {
    const { vehicles, isLoading, error, isCreating, createVehicle } = useVehicle();
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    return (
                        <div
                            ...
                            className={`cursor-pointer bg-white border-2 rounded-2xl p-5 transition-all duration-200 flex flex-col gap-4 relative group ${
                                isSelected ? 'border-blue-600 ring-4 ring-blue-50 shadow-sm' : 'border-gray-100 hover:border-blue-200'
                            }`}
                        >
                            <div className="flex items-center gap-4">
                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors ${
                                    isSelected ? 'bg-blue-50 text-blue-600' : 'bg-gray-50 text-gray-400'
                                }`}>
                                    <Car className="w-6 h-6" />
                                </div>
                                <div>
                                    <h4 className="font-bold text-gray-900">{vehicle.vehicleName}</h4>
                                    <p className="text-xs text-gray-500 font-medium uppercase tracking-tighter">{vehicle.type}</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 text-sm pt-3 border-t border-gray-50">
                                <div>
                                    ...
                                    <span className="font-bold text-gray-700">{vehicle.color}</span>
                                </div>
                                <div>
                                    <span className="block text-[10px] text-gray-400 font-bold uppercase">License Plate</span>
                                    <span className="font-black text-blue-600 tracking-wider">{vehicle.licensePlate}</span>
                                </div>
                            </div>
                        </div>
                    );
                ...
                >
                    ...
                    <span className="text-sm font-bold text-gray-400 group-hover:text-blue-600 transition-colors">Add Vehicle</span>
                </div>
            </div>
