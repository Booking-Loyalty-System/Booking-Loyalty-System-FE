...
import { 
    CheckCircle2, MapPin, Package, Car, Clock, 
    Award, Gift, Mail, ArrowRight, Loader2, X, CreditCard, Wallet, Smartphone, Zap
} from 'lucide-react';
...
export const SummaryStep: React.FC<SummaryStepProps> = ({ 
    branch, pkg, vehicle, date, slot, paymentMethod, pointsUsed, onSuccess, onEdit 
}) => {
    ...
    const estimatedPoints = Math.round(finalTotal * (loyaltyInfo?.multiplier || 1.0));
    ...
    const getPaymentIcon = () => {
        switch (paymentMethod) {
            ...
            case 'qr': return <Smartphone className="w-5 h-5" />;
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="text-center space-y-2">
                <h2 className="text-3xl font-black text-slate-900 tracking-tight">Review & Confirm</h2>
                <p className="text-slate-500 font-medium">Please double check your appointment details</p>
            </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-slate-50 rounded-3xl p-6 border border-slate-100 flex items-start gap-5 group hover:bg-white hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300">
                        ...
                        <div className="flex-1">
                            <div className="flex justify-between items-start">
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Location & Schedule</span>
                                <button onClick={() => onEdit('branch')} className="text-[10px] font-black text-blue-600 hover:text-blue-700 uppercase tracking-widest">Change</button>
                            </div>
                            <h4 className="font-black text-slate-900 text-lg mt-1">{branch.name}</h4>
                            ...
                            <div className="mt-4 flex items-center justify-between">
                                <div className="flex items-center gap-2 text-blue-700 font-black text-sm">
                                    ...
                                    <span>{format(new Date(date), 'EEEE, MMM dd')} @ {slot}</span>
                                </div>
                                <button onClick={() => onEdit('slot')} className="text-[10px] font-black text-blue-600 hover:text-blue-700 uppercase tracking-widest">Edit Time</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-slate-50 rounded-3xl p-6 border border-slate-100 space-y-4">
                            <div className="flex justify-between items-start">
                                ...
                                <button onClick={() => onEdit('vehicle')} className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Edit</button>
                            </div>
                            <div>
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Vehicle</span>
                                <h4 className="font-black text-slate-900 mt-1">{vehicle.vehicleName}</h4>
                                <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded mt-1 inline-block">{vehicle.licensePlate}</span>
                            </div>
                        </div>
                        ...
                        <div className="bg-slate-50 rounded-3xl p-6 border border-slate-100 space-y-4">
                            <div className="flex justify-between items-start">
                                ...
                                <button onClick={() => onEdit('package')} className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Edit</button>
                            </div>
                            <div>
                                ...
                                <span className="text-[10px] font-black text-purple-600 bg-purple-50 px-2 py-0.5 rounded mt-1 inline-block">${pkg.price}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-slate-50 rounded-3xl p-6 border border-slate-100 flex items-center justify-between group hover:bg-white hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300">
                        <div className="flex items-center gap-5">
                            ...
                            <div>
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Payment Method</span>
                                <h4 className="font-black text-slate-900 text-lg mt-0.5">{getPaymentLabel()}</h4>
                            </div>
                        </div>
                        <button onClick={() => onEdit('payment')} className="text-[10px] font-black text-blue-600 hover:text-blue-700 uppercase tracking-widest">Change</button>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-slate-900 rounded-[40px] p-8 text-white relative overflow-hidden group shadow-2xl shadow-slate-200">
                        ...
                        <div className="flex items-center gap-4 mb-6">
                            ...
                            <span className="font-black tracking-tight text-lg">Rewards Summary</span>
                        </div>

                        <div className="grid grid-cols-2 gap-6">
                            <div className="space-y-1">
                                <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">You will earn</p>
                                <div className="flex items-baseline gap-2">
                                    <span className="text-3xl font-black text-white">+{estimatedPoints}</span>
                                    <span className="text-[10px] font-black text-blue-400 uppercase">pts</span>
                                </div>
                            </div>
                            {pointsUsed > 0 && (
                                <div className="space-y-1">
                                    <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">Points used</p>
                                    <div className="flex items-baseline gap-2">
                                        <span className="text-3xl font-black text-rose-400">-{pointsUsed}</span>
                                        <span className="text-[10px] font-black text-rose-400 uppercase">pts</span>
                                    </div>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="bg-white border-2 border-slate-50 rounded-[40px] p-8 space-y-4 shadow-sm">
                        ...
                        <div className="pt-6 border-t border-slate-100 flex justify-between items-end">
                            <div>
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Final Amount</p>
                                <p className="text-4xl font-black text-blue-600 tracking-tighter">${finalTotal.toFixed(2)}</p>
                            </div>
                            <button
...
export const BookingSuccessModal: React.FC<SuccessModalProps> = ({ bookingCode, onClose }) => {
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-500">
            <div className="bg-white rounded-[48px] max-w-lg w-full p-12 text-center relative shadow-[0_32px_64px_-12px_rgba(0,0,0,0.2)] animate-in zoom-in-95 duration-500">
                ...
                <h2 className="text-4xl font-black text-slate-900 mb-3 tracking-tight">Booking Confirmed!</h2>
                <p className="text-slate-500 font-bold mb-10 px-4">
                    Your car wash is scheduled. We've sent the details and a QR code to your email.
                </p>

                <div className="bg-blue-600 rounded-[32px] p-8 mb-10 shadow-2xl shadow-blue-500/30 group">
                    <span className="text-[10px] font-black text-blue-100 uppercase tracking-[0.3em] mb-2 block opacity-80">Appointment ID</span>
                ...
                <div className="grid grid-cols-2 gap-4 mb-10">
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex items-center gap-3 text-left">
                        ...
                        <p className="text-[10px] font-black text-slate-500 leading-tight uppercase tracking-wider">Check your Inbox for QR</p>
                    </div>
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex items-center gap-3 text-left">
                        <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center text-emerald-600 shadow-sm shrink-0">
                            <Zap className="w-4 h-4" />
                        </div>
                        <p className="text-[10px] font-black text-slate-500 leading-tight uppercase tracking-wider">Points Added to Account</p>
                    </div>
                </div>
